class ProxmarkFirmwareUltimate < Formula
  ...
  bottle do
    ...
    sha256 cellar: :any_skip_relocation, arm64_tahoe:   "b087573ee936da34974e3e8eac94df561f43d18df8598beff789974af05f449e"
    sha256 cellar: :any_skip_relocation, arm64_sequoia: "7085d2d64fcbfab8b494bb98f38d2ff8f92dc580e071503efd9f113f3f66431d"
  end
  ...
  def install
    ...
    %w[all bootrom fullimage].each do |mode|
      ...
      wrapper = bin/"pm3-flash-ultimate-#{mode}"
      wrapper.write <<~SH
        #!/bin/sh
        set -eu
  ...
  def caveats
    ...
    <<~EOS
      ...
      Use a client matching the firmware version. Device-specific launchers:
        pm3-flash-ultimate-bootrom
        pm3-flash-ultimate-fullimage
        pm3-flash-ultimate-all
  ...
  test do
    ...
    ENV["PATH"] = "helpers"
    output = shell_output("#{bin}/pm3-flash-ultimate-all --list", 37)
    assert_equal ["all", firmware.realpath.to_s, "--list"], output.lines.map(&:chomp)

    ENV["PATH"] = (testpath/"no-client").to_s
    output = shell_output("#{bin}/pm3-flash-ultimate-all 2>&1", 127)
