class ProxmarkFirmwareRdv4 < Formula
  desc "RRG/Iceman firmware for Proxmark3 RDV4"
  ...
  def caveats
    firmware = opt_share/"proxmark3/firmware/rdv4"
    <<~EOS
      Firmware for Proxmark3 RDV4 is installed in:
        #{firmware}
      ...
      Use a client matching the firmware version. Device-specific launchers:
        pm3-flash-rdv4-bootrom
        pm3-flash-rdv4-fullimage
        pm3-flash-rdv4-all
  ...
  test do
    ...
    ENV["PATH"] = "helpers"
    output = shell_output("#{bin}/pm3-flash-rdv4-all --list", 37)
    assert_equal ["all", firmware.realpath.to_s, "--list"], output.lines.map(&:chomp)

    ENV["PATH"] = (testpath/"no-client").to_s
    output = shell_output("#{bin}/pm3-flash-rdv4-all 2>&1", 127)
