...
The workflow verifies asset checksums, uploads all bottles to one draft release
and generates their build provenance before publishing it. Its release tag
includes the publication run and attempt, so retries never modify an immutable
release. Failed attempts may leave an unused draft or release; they do not
replace an existing release's assets.

The workflow commits bottle metadata pointing to that release and pushes to the
default branch, then tags that exact published commit. Tagging is invoked explicitly
